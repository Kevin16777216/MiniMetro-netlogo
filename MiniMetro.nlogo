...
to highlightLine[ox oy px py]
  ...
  set pen-size (size / 3)
  ifelse isReal[
    set color (item colorLine colorPalette) + isHighlighted
  ][
    set color gray
  ]
  pd
...
to isCloseToPath
 ...
 let near getNearestSegment
  let minDist 0
  if near != nobody[
    ask near[
      set minDist distPoint
      ;show minDist
    ]
  ]
  if (minDist <= lineThreshold) and near != nobody[
    ask near[
      if myPath != nobody[
      ask myPath[
        set ishighlighted 3
...
to-report distPoint
  ifelse (mouse-xcor > (minX - lineThreshold)) and (mouse-xcor < (maxX + lineThreshold)) and (mouse-ycor > (minY - lineThreshold)) and (mouse-ycor < (maxY + lineThreshold))[
    report (distPointSeg Mseg Bseg mouse-xcor mouse-ycor self)
  ][
    report 20000
  ]
end
to-report getNearestSegment
  report min-one-of segments [distMouse]
end
to checkTrain
  if isDraggingTrain [
    isCloseToPath
