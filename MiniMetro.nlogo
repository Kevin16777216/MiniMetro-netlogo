...
globals[
  ...
  playButton;
  isPaused;
  trainSpeed; how fast do trains pickup people
  availableTrains;
...
to setup
  ...
  while [m < 10][
    ...
    while [not canSpawn][
      ...
      ask nearest[
        if (distancexy rx ry) > (trackWidth * 3) and (ry > (min-pycor + (2 * trackWidth))) and (abs(rx) < (max-pxcor - (trackWidth * 2))) and (abs(ry) < (max-pycor - (trackWidth * 2))) [
           set canSpawn true
        ]
      ]
      ][
        if (abs(rx) < (max-pxcor - (trackWidth * 2))) and (abs(ry) < (max-pycor - (trackWidth * 2)))[
          set canSpawn true
        ]
      ]
    ]
...
to-report getNearestSegment [x y]
...
to updateTrainButton
  let return false
  ask trainButton[
    set return ((distancexy mouse-xcor mouse-ycor) < size) and mouse-down? and mouse-inside? and (not lastFrameClicked)
...
to doAI [currentStation destinationType]
end
@#$#@#$#@
GRAPHICS-WINDOW
195
...
Rectangle -7500403 true true 0 240 315 300
